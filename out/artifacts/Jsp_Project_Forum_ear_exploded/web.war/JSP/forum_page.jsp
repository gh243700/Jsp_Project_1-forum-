...
<article>
    ...
    <form action="forum/${requestScope.title}" method="get" class="container-fluid row">
    ...
    <section class="container-fluid">
        <div class="btn-group bg-light">
            ...
            <c:if test="${requestScope.PAGE_INDEX != 0}">
                <a href="?page=${requestScope.PAGE_INDEX}" class="btn btn-danger btn-sm" type="button">PREV</a>
                <a href="?page=${requestScope.PAGE_INDEX}" class="btn btn-danger btn-sm"
                   type="button">${requestScope.PAGE_INDEX}</a>
            </c:if>
            <button class="btn btn-danger btn-sm" type="button" disabled>${requestScope.PAGE_INDEX+1}</button>
            <c:forEach begin="${requestScope.PAGE_INDEX+2}" end="${requestScope.PAGE_INDEX+5}"
                       varStatus="loop">
                <a href="?page=${loop.index}" class="btn btn-danger btn-sm" type="button">${loop.index} </a>
            </c:forEach>
        ...
        <div class="list-group">
            <c:forEach var="topic" items="${requestScope.topics}">
                <div class="list-group-item">
                    <a href="/project_web/topic?attr=${topic.id}-${topic.title.replace(" ","-")}">${topic.title}</a>
                    <br>
